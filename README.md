...
| `AutogramVector256` | `byte[]` | `HashSet<ByteHistoryKey32>` (max 32 chars) | Offsets all mismatched counts | Based on `V5i`, but precomputes each flattened numeric-count row as a single `Vector256<byte>` with zero padding up to 32 bytes; included in the benchmark suite **active default** |
